...
def swichMatrix(lpl,M):#switches colors of appropriate entries, returns new live pixel list
    ...
    sdpl=deadPixelsWith3Neighbors(lpl,len(M),len(M[0]))#list of dead pixel list to switch
...
def callback(event):
    ...
    if event.x<startW or event.x>startW+dW or event.y<startH or event.y>startH+dH:
        ...
        i=(event.x-startW)/deltaW
        j=(event.y-startH)/deltaH
...
for i in xrange(numW):
    ...
    sc=[]
    #height
    for j in xrange(numH):
        sy=startW+i*deltaW
        sx=startH+j*deltaH
        Id=canvas.create_rectangle(sy,sx,sy+deltaW,sx+deltaH,fill="white")
